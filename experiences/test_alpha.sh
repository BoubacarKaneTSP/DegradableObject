...
do
  ...
  #  for nbThread in 1 16 48
    ...
    do
	  ...
	  ./test.sh -c CounterIncrementOnly -s SegmentedHashSet -q Queue -m Map -t Retwis -r "$ratio" -p -e -w $benchmarkTime -u $warmingUpTime -h "Q_M_C_$str_alpha" -y $nbUsersInit -d $nbUsersInit -i $nbOps -b -g $nbThread -A $alpha -z
      done
    done
