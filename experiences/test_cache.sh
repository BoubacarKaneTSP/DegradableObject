...
for nbUsersInit in "${nbUserInits[@]}"
do
  for (( i=0; i<length; i++ ));
  do
    ...
    do
      ...
      do
        ...
        python3 analyse_perf.py perf.log "false" "${tags[i]}" $nbThread $nbUsersInit

        done
      done

      python3 compute_avg_throughput.py $nbUsersInit "${tags[i]}" "${nbThreads[@]}" $completion_time
      python3 analyse_perf.py perf.log "true" "${tags[i]}" "${nbThreads[@]}" $nbUsersInit
