...
for nbUsersInit in 10000 100000
do
  #  Cleaning old file
...
#  python3 rm_file.py $nbUsersInit "Q_M_S_C"
  ...
  for nbThread in 2 4 8 16 32 48 64 70 86 96
...
#      python3 analyse_perf.py perf.log "false" "SEQ" $nbThread $nbUsersInit
      ...
      perf stat --no-big-num -d -e cache-references,cache-misses,branches,branch-misses,cycles,instructions,l1d_pend_miss.pending_cycles_any,l2_rqsts.all_demand_miss,cycle_activity.stalls_total -o perf.log ./test.sh -c CounterIncrementOnly -s ExtendedSegmentedHashSet -q QueueMASP -m ExtendedSegmentedHashMap -t Retwis -r "$ratio" -p -e -w $benchmarkTime -u $warmingUpTime -h "Q_M_C" -y $nbUsersInit -d $nbUsersInit -i $nbOps -b -g $nbThread #-j #-z $nbOps
