...
do
  for type in 'SetSnapshot' 'ListSnapshot' 'SetSnapshotV2' 'ListSnapshotV2'
...
#  for type in 'DegradableCounter'
    do
    echo $type $ratio
    CLASSPATH=../java/target/*:../java/target/lib/* java eu.cloudbutton.dobj.Benchmark -type $type -ratios $ratio -nbTest 1 -time 90 -wTime 25 > "results_${type}_ratio_write_${ratio}_SR.txt"
  done
done
