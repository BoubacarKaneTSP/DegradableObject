...
for type in 'ConcurrentHashMap' 'DegradableMap' # 'ConcurrentSkipListSet' 'DegradableSet'
  do
    echo $type $ratio
    CLASSPATH=../java/target/*:../java/target/lib/* java -XX:+UseNUMA -XX:+UseG1GC eu.cloudbutton.dobj.Benchmark.Benchmark -type $type -ratios $ratio -nbTest 5 -time 10 -wTime 5 #> "results_${type}_ratio_write_${ratio}_SR2.txt"
    echo " "
  done
done