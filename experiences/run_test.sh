...
perf stat -B -e cache-references,cache-misses ./test.sh -c Counter -t Microbenchmark -p -r "50 50 0" -w $benchmarkTime -u $warmingUpTime -n $nbTest -i $initSize -d $range
perf stat -B -e cache-references,cache-misses ./test.sh -c CounterIncrementOnly -t Microbenchmark -p -r "50 50 0" -w $benchmarkTime -u $warmingUpTime -n $nbTest -i $initSize -d $range
perf stat -B -e cache-references,cache-misses ./test.sh -c LongAdder -t Microbenchmark -p -r "50 50 0" -w $benchmarkTime -u $warmingUpTime -n $nbTest -i $initSize -d $range
