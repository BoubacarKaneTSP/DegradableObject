 ...
 */
public
class ThreadGroupCustom implements ThreadCustom.UncaughtExceptionHandler {
    private final ThreadGroupCustom parent;
    String name;
    int maxPriority;
    boolean destroyed;
    boolean daemon;
    ...
    int nthreads;
    ThreadCustom threads[];

    int ngroups;
    ThreadGroupCustom groups[];

    /**
     * Creates an empty Thread group that is not in any Thread group.
     * This method is used to create the system Thread group.
     */
    private ThreadGroupCustom() {     // called from C code
        this.name = "system";
        this.maxPriority = ThreadCustom.MAX_PRIORITY;
        this.parent = null;
    }
     ...
     * @since   1.0
     */
    public ThreadGroupCustom(String name) {
        this(ThreadCustom.currentThread().getThreadGroup(), name);
    }
     ...
     * @since   1.0
     */
    public ThreadGroupCustom(ThreadGroupCustom parent, String name) {
        this(checkParentAccess(parent), parent, name);
    }

    private ThreadGroupCustom(Void unused, ThreadGroupCustom parent, String name) {
        this.name = name;
     ...
     *                                thread in the specified thread group.
     */
    private static Void checkParentAccess(ThreadGroupCustom parent) {
        parent.checkAccess();
        return null;
    }
     ...
     * @since   1.0
     */
    public final ThreadGroupCustom getParent() {
        if (parent != null)
            parent.checkAccess();
        return parent;
    }
     ...
     * {@link ThreadCustom#MIN_PRIORITY} or greater than
     * {@link ThreadCustom#MAX_PRIORITY}, the maximum priority of the group
     * remains unchanged.
     * <p>
    ...
    public final void setMaxPriority(int pri) {
        int ngroupsSnapshot;
        ThreadGroupCustom[] groupsSnapshot;
        synchronized (this) {
            checkAccess();
            if (pri < ThreadCustom.MIN_PRIORITY || pri > ThreadCustom.MAX_PRIORITY) {
                return;
            }
     ...
     * @since   1.0
     */
    public final boolean parentOf(ThreadGroupCustom g) {
     ...
     *
     * <blockquote>
     * {@linkplain #enumerate(ThreadCustom[], boolean) enumerate}{@code (list, true)}
     * </blockquote>
     *
     * @param  list
     ...
     * @since   1.0
     */
    public int enumerate(ThreadCustom list[]) {
        checkAccess();
     ...
     * @since   1.0
     */
    public int enumerate(ThreadCustom list[], boolean recurse) {
        checkAccess();
        return enumerate(list, 0, recurse);
    }

    private int enumerate(ThreadCustom list[], int n, boolean recurse) {
        int ngroupsSnapshot = 0;
        ThreadGroupCustom[] groupsSnapshot = null;
        synchronized (this) {
            if (destroyed) {
                return 0;
            }
     ...
     *
     * <blockquote>
     * {@linkplain #enumerate(ThreadGroupCustom[], boolean) enumerate}{@code (list, true)}
     * </blockquote>
     *
     * @param  list
     ...
     * @since   1.0
     */
    public int enumerate(ThreadGroupCustom list[]) {
        checkAccess();
        return enumerate(list, 0, true);
    }
     ...
     * @since   1.0
     */
    public int enumerate(ThreadGroupCustom list[], boolean recurse) {
        checkAccess();
        return enumerate(list, 0, recurse);
    }

    private int enumerate(ThreadGroupCustom list[], int n, boolean recurse) {
        int ngroupsSnapshot = 0;
        ThreadGroupCustom[] groupsSnapshot = null;
        synchronized (this) {
            if (destroyed) {
     ...
     *     {@link ThreadCustom#stop} for details.
     */
    @Deprecated(since="1.2")
    public final void stop() {
        if (stopOrSuspend(false))
            ThreadCustom.currentThread().stop();
    }
     ...
     *     {@link ThreadCustom#suspend} for details.
     */
    @Deprecated(since="1.2")
    @SuppressWarnings("deprecation")
    public final void suspend() {
        if (stopOrSuspend(true))
            ThreadCustom.currentThread().suspend();
    }
    ...
    private boolean stopOrSuspend(boolean suspend) {
        ...
        ThreadCustom us = ThreadCustom.currentThread();
        int ngroupsSnapshot;
        ThreadGroupCustom[] groupsSnapshot = null;
        synchronized (this) {
            checkAccess();
     ...
     *       deadlock-prone.  See {@link ThreadCustom#suspend} for details.
     */
    @Deprecated(since="1.2")
    @SuppressWarnings("deprecation")
     ...
     */
    private final void add(ThreadGroupCustom g){
        synchronized (this) {
            if (destroyed) {
                throw new IllegalThreadStateException();
            }
            if (groups == null) {
                groups = new ThreadGroupCustom[4];
     ...
     */
    private void remove(ThreadGroupCustom g) {
        synchronized (this) {
            if (destroyed) {
                return;
            }
     ...
     */
    void add(ThreadCustom t) {
        synchronized (this) {
            if (destroyed) {
                throw new IllegalThreadStateException();
            }
            if (threads == null) {
                threads = new ThreadCustom[4];
     ...
     */
    void threadStartFailed(ThreadCustom t) {
        synchronized(this) {
            remove(t);
            nUnstartedThreads++;
        }
     ...
     */
    void threadTerminated(ThreadCustom t) {
        synchronized (this) {
            remove(t);
     ...
     */
    private void remove(ThreadCustom t) {
        synchronized (this) {
            if (destroyed) {
                return;
            }
     ...
     * does not have a specific {@link ThreadCustom.UncaughtExceptionHandler}
     * installed.
     * <p>
     ...
     *     {@linkplain ThreadCustom#getDefaultUncaughtExceptionHandler default
     ...
     *     ThreadCustom#getName getName} method, and a stack backtrace,
     *     using the {@code Throwable}'s  method, is
     ...
     * @since   1.0
     */
    public void uncaughtException(ThreadCustom t, Throwable e) {
        if (parent != null) {
            parent.uncaughtException(t, e);
        } else {
            ThreadCustom.UncaughtExceptionHandler ueh =
                    ThreadCustom.getDefaultUncaughtExceptionHandler();
            if (ueh != null) {
