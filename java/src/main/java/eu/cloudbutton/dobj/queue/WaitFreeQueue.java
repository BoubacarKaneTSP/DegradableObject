...
public class WaitFreeQueue<T> implements Queue<T> {
	...
	public int size() {
		...
		return (int) (putIndex.get() - popIndex.get());
	}
	...
	static {
		try {
			putNode_offset = _unsafe.objectFieldOffset(WaitFreeQueue.class.getDeclaredField("putNode"));
			popNode_offset = _unsafe.objectFieldOffset(WaitFreeQueue.class.getDeclaredField("popNode"));
			ringTail_offset = _unsafe.objectFieldOffset(WaitFreeQueue.class.getDeclaredField("ringTail"));
		} catch (Exception e) {
			throw new Error(e);
		}
	}
   ...
   */
	public void enqueue(T item, Handle<T> handle) {
		...
		Cell<T> cell = new Cell<>(item), local;
		for(; times-- > 0;) {
   ...
   */
	...
	public Handle<T> register() {
		...
		Handle<T> handle = new Handle<>(null, null);
